...
router.post('/', authenticateToken, requireCapability(CAPABILITIES.PAYROLL_RUN), async (req, res) => {
  try {
    ...
    if (approvedReimbursements.rows.length > 0) {
      ...
      await query(
        ...
        [run.id, reimbursementIds, tenantId]
      );
    }
    ...
    await audit({
      actorId: req.user.id,
      action: 'reimbursement_run_created',
      entityType: 'reimbursement_run',
      entityId: run.id,
      details: { total_claims: totalClaims, total_amount: totalAmount },
    });
...
router.get('/', authenticateToken, requireCapability(CAPABILITIES.PAYROLL_READ), async (req, res) => {
  try {
    const tenantId = await getTenantIdForUser(req.user.id);
    if (!tenantId) {
      return res.status(403).json({ error: 'No organization found' });
    ...
    const runs = await query(
      `SELECT 
        rr.*,
        COALESCE(p.first_name || ' ' || p.last_name, p.first_name, p.last_name, 'System') as created_by_name
...
router.get('/:id', authenticateToken, requireCapability(CAPABILITIES.PAYROLL_READ), async (req, res) => {
  try {
    const tenantId = await getTenantIdForUser(req.user.id);
    if (!tenantId) {
      return res.status(403).json({ error: 'No organization found' });
...
router.post('/:id/process', authenticateToken, requireCapability(CAPABILITIES.PAYROLL_RUN), async (req, res) => {
  try {
    const tenantId = await getTenantIdForUser(req.user.id);
    if (!tenantId) {
      return res.status(403).json({ error: 'No organization found' });
    ...
    await audit({
      actorId: req.user.id,
      action: 'reimbursement_run_processed',
      entityType: 'reimbursement_run',
      entityId: id,
      details: { total_claims: run.total_claims, total_amount: run.total_amount },
    });
...
router.get('/:id/export/bank-file', authenticateToken, requireCapability(CAPABILITIES.PAYROLL_READ), async (req, res) => {
  try {
    const tenantId = await getTenantIdForUser(req.user.id);
    if (!tenantId) {
      return res.status(403).json({ error: 'No organization found' });
